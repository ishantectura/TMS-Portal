...
public partial class PendingRequest : System.Web.UI.Page
{
    ...
    protected void Page_Init(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            if (Session["UserType"].ToString() == "Transporter"  )
            {
                ...
                //Added by Jyothi
                this.grdPendingRequestExcel.Columns[(this.grdPendingRequestExcel.Columns.Count - 4)].Visible = false;
                this.grdPendingRequestExcel.Columns[(this.grdPendingRequestExcel.Columns.Count - 1)].Visible = false;
    ...
    {
        BindGridview(txtcity.Text,txtrequestno.Text,txttransportercode.Text);
    ...
    private void SubmitTransporterRequest()
    {
        ...
        foreach (GridViewRow row in grdPendingRequest.Rows)
        {
            ...
            if (chkSelectedRow.Checked)
            {
                ...
                dr["ID"] = key;
                dr["ReqNo"] =  grdPendingRequest.Rows[row.RowIndex].Cells[1].Text;
        ...
        foreach (GridViewRow row1 in grdPendingRequest.Rows)
        {
            ...
            if (chkSelectedRow.Checked)
            {
                ...
                dr1["ReqNo"] = grdPendingRequest.Rows[row1.RowIndex].Cells[1].Text;
    ...
    private void BindGridview(string city,string requestNo , string transportNo)
    {

        if (Session["UserType"].ToString() == "Transporter")
        {
                grdPendingRequest.DataSource = (new PendingRequests()).GetPendingRequests(Session["UserName"].ToString(), city,requestNo, transportNo);
                grdPendingRequestExcel.DataSource = (new PendingRequests()).GetPendingRequests(Session["UserName"].ToString(), city, requestNo, transportNo);
        }
        if ((Session["UserType"].ToString() == "CIPL") || (Session["UserType"].ToString() == "Admin")) //Modified by Jyothi
        {
                grdPendingRequest.DataSource = (new PendingRequests()).GetPendingRequestsForCIPL(Session["UserName"].ToString(), city, requestNo, transportNo);
            grdPendingRequestExcel.DataSource = (new PendingRequests()).GetPendingRequestsForCIPL(Session["UserName"].ToString(), city, requestNo, transportNo);
        }

        grdPendingRequest.DataBind();
        grdPendingRequestExcel.DataBind();//Added by Jyothi
 ...
 protected void chkboxSelectAll_CheckedChanged(object sender, EventArgs e)
    ...
    //Added by Jyothi
     ...
     private void ExportGridToExcel(GridView gridview)
     {
         ...
         Response.Charset = "";
         string FileName = "PendingRequest" + DateTime.Now + ".xls";
    ...
    protected void btnGo_Click(object sender, EventArgs e)
    {
        if (txtcity.Text != "" || txtrequestno.Text!="" || txttransportercode.Text!="")
            BindGridview(txtcity.Text.ToString(),txtrequestno.Text.ToString(),txttransportercode.Text.ToString());
        else
            BindGridview("","","");
    }
}